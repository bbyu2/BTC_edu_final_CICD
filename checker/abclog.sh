...
find /home/ec2-user/log/nginx/* -mtime +30 -exec sh -c "aws s3 cp {} s3://abcbit-log/All_log/$TODAY/nginx/ --storage-class STANDARD --profile abc-profile; rm -f {};" \;
find /home/ec2-user/log/gunicorn/* -mtime +30 -exec sh -c "aws s3 cp {} s3://abcbit-log/All_log/$TODAY/gunicorn/ --storage-class STANDARD --profile abc-profile; rm -f {};" \;
